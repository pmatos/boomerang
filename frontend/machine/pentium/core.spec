...
# Copyright (C) 1998-1999,2001, The University of Queensland
#
...
fieldinfo reg_opcode is [names [ eax ecx edx ebx esp ebp esi edi ]]
...
patterns aw is AddrPrefix
         ad is epsilon
         av is aw | ad
patterns rep is REP | REPNE
patterns strings   is  CMPSB | ov ; CMPSv | LODSB | ov ; LODSv |
                       MOVSB | ov ; MOVSv |
                       SCASB | ov ; SCASv | STOSB | ov ; STOSv
#patterns repstrings is rep ; strings
...
FCOMI.ST.STi   idx  is  DB ; mod = 3 & reg_opcode = 6 & r_m = idx
FCOMIP.ST.STi  idx  is  DF ; mod = 3 & reg_opcode = 6 & r_m = idx
FUCOMI.ST.STi  idx  is  DB ; mod = 3 & reg_opcode = 5 & r_m = idx
FUCOMIP.ST.STi idx  is  DF ; mod = 3 & reg_opcode = 5 & r_m = idx
FCOMPP
FCOS
FDECSTP
...
rep^.MOVSB      is rep ; MOVSB
rep^.MOVSv^ov   is rep; ov; MOVSv 
...
RDMSR
#REP            # These are now prefixes, like OpPrefix
#REPNE
# The below doesn't to what I would like; the [name] is always "repstrings"
repstrings      is rep ; strings
RET
RET.far
...
rep^.STOSB      is rep ; STOSB
rep^.STOSv^ov   is rep; ov; STOSv 
