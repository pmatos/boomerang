...
{
	GotoStatement *stmt_jump = static_cast<GotoStatement *>(inst.rtl->getList().back());
			...
			case SU:
				{
					...
					PBB pBB = cfg->newBB(BB_rtls, ONEWAY, 1);
					handleBranch(stmt_jump->getFixedDest(), pBF->getLimitTextHigh(), pBB, cfg, targetQueue);
					...
					case NCT:
						{
							...
							if (last->getKind() == STMT_CALL) {
								...
								case_SD(address, pBF->getTextDelta(), pBF->getLimitTextHigh(), inst, delay_inst, BB_rtls, cfg, targetQueue, os);
					...
					case SU:
						{
							...
							if (last->getKind() == STMT_CALL) {
								...
								// to NULL so that a new list of RTLs will be created for the next BB.
								BB_rtls = NULL;
								address = address + 8;
			...
			case DD:
				{
					...
					case NCT:
						{
							sequentialDecode = case_DD(address, pBF->getTextDelta(), inst, delay_inst, BB_rtls, targetQueue, proc, callList);
						}
						break;
					default:
						case_unhandled_stub(address);
			...
			case SCD:
				{
					...
					case NCT:
						{
							sequentialDecode = case_SCD(address, pBF->getTextDelta(), pBF->getLimitTextHigh(), inst, delay_inst, BB_rtls, cfg, targetQueue);
						}
						break;
					default:
			...
			case SCDAN:
				{
					...
					case NCT:
						{
							sequentialDecode = case_SCDAN(address, pBF->getTextDelta(), pBF->getLimitTextHigh(), inst, delay_inst, BB_rtls, cfg, targetQueue);
						}
						break;
...
void SparcFrontEnd::gen32op32gives64(OPER op, std::list<RTL *> *lrtl, ADDRESS addr)
{
	...
	// r[tmp] = r8 op r9
	Assign *a = new Assign(Location::tempOf(new Const("tmp")),
	                       new Binary(op,  // opMult or opMults
	                                  Location::regOf(8),
	                                  Location::regOf(9)));
	ls->push_back(a);
